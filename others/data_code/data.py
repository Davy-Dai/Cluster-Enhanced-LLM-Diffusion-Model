...
def print_click_statistics(df, dataset_name):
    """
    统计并打印指定数据集的点击/未点击占比
    参数：
        df: 数据集切片（pandas DataFrame）
        dataset_name: 数据集名称（如Train/Dev/Test）
    """
    total = len(df)
    if total == 0:
        print(f"\n📊 {dataset_name} 数据集统计：")
    ...
    non_click_count = total - click_count
    click_ratio = (click_count / total) * 100
    non_click_ratio = (non_click_count / total) * 100

    # 格式化输出
    print(f"\n📊 {dataset_name} 数据集统计：")
    print(f"   总样本数：{total:,}")  # 千分位分隔
    print(f"   点击数 (is_click=1)：{click_count:,}，占比：{click_ratio:.2f}%")
    print(f"   未点击数 (is_click=0)：{non_click_count:,}，占比：{non_click_ratio:.2f}%")
...
def main():
    ...
    # 2. 加载token ID到64维向量的映射（核心修正点）
    token_vector_map = load_token_vector_map(INPUT_FOLDER)

    # ===========================================
    # 第一步：处理交互数据 (train/dev/test.json)
    # ===========================================
    ...
    try:
        ...
        # 时序切分 (80% train, 10% dev, 10% test)
        total = len(output_inter)
        train_end = int(total * 0.8)
        dev_end = train_end + int(total * 0.1)

        # 获取各数据集切片
        train_slice = output_inter.iloc[:train_end]
        dev_slice = output_inter.iloc[train_end:dev_end]
        test_slice = output_inter.iloc[dev_end:]

        # 统计并打印各数据集点击占比（核心新增逻辑）
        print_click_statistics(train_slice, "Train")
        print_click_statistics(dev_slice, "Dev")
        print_click_statistics(test_slice, "Test")

        # 转换为字典列表保存
        train_data = train_slice.to_dict(orient='records')
        dev_data = dev_slice.to_dict(orient='records')
        test_data = test_slice.to_dict(orient='records')
    ...
    try:
        ...
        for _, row in answer_df.iterrows():
            ...
            # 计算token的64维平均词向量（核心修正：使用token映射字典）
